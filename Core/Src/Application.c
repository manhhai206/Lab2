...
packet_t global_date_packet;
packet_t global_time_packet;
packet_t global_adc_packet;
packet_t global_string_packet;
packet_t global_button_packet;
packet_t global_temperature_packet;
...
void send_string_data(uint16_t string_len, uint8_t string[1024])
{
    hello_world_stream_data_t string_data;
    string_data.string_len = string_len;
    memcpy(string_data.string, string, string_len);

    send_packet_data(&global_string_packet, &string_data, sizeof(hello_world_stream_data_t));
}
